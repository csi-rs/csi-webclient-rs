...
use crate::profile::{ClientProfile, StandardClientProfile};
use crate::state::{
    AppState, ConfigSnapshotFile, ControlStatus, DeviceAction, DeviceConfig, DeviceForms,
    DeviceInfo, DeviceListEntry, DeviceState, PairingPreset, ServerStatus, Tab, TrafficForm,
    UserIntent, WiFiForm, WiFiMode, WifiProtocol,
};
use crate::ui;
use eframe::egui;
...
pub struct CsiClientApp {
    ...
    /// Sequential config steps (device id, action); drained on each 2xx
    /// response. Used by pairing presets and full-config apply/copy.
    step_queue: Vec<(String, DeviceAction)>,
    /// Status/event message emitted when the step queue completes.
    step_queue_done_message: String,
    /// Injectable per-deployment behavior (protocols, presets, export labels).
    profile: Arc<dyn ClientProfile>,
}

impl CsiClientApp {
    /// Create a new app instance with default state, a running core worker, and
    /// the no-op [`StandardClientProfile`].
    pub fn new(cc: &eframe::CreationContext<'_>) -> Self {
        Self::with_profile(cc, Arc::new(StandardClientProfile))
    }

    /// Create a new app instance with an injected [`ClientProfile`].
    ///
    /// A companion binary calls this to add chip- or standard-specific behavior
    /// (extra PHY protocols, CSI presets, `data_format` labeling) without the
    /// open library naming any of it.
    pub fn with_profile(
        ...
        Self {
            ...
            step_queue: Vec::new(),
            step_queue_done_message: String::new(),
            profile,
        }
    }
        ...
        match action {
            ...
            DeviceAction::SetTraffic(traffic) => {
                if let Some(frequency_hz) = parse_required_u64(&traffic.frequency_hz) {
                    ...
                    // firmware ≥ 0.7.0: the flag is meaningless elsewhere,
            ...
            DeviceAction::SetCsi(csi) => {
                if let Some(val_scale_cfg) = parse_required_u32(&csi.val_scale_cfg) {
                    ...
                    self.set_error("val_scale_cfg must be a valid u32 number");
                }
            }
            ...
            DeviceAction::SaveConfigFile { path } => self.save_config_file(&id, &path),
            DeviceAction::LoadConfigFile { path } => self.load_config_file(&id, &path),
            DeviceAction::ApplyFullConfig => self.apply_full_config(&id),
            DeviceAction::CopyConfigFrom { source_id } => self.copy_config_from(&id, &source_id),
        }
    }

    /// Write `id`'s form values to a JSON snapshot file.
    ///
    /// An empty path auto-names the file in the export directory and writes
    /// the chosen path back into the Config-tab field.
    fn save_config_file(&mut self, id: &str, path: &str) {
        ...
        let path = if path.trim().is_empty() {
            ...
            format!("{dir}/csi_config_{id}_{stamp}.json")
        } else {
            path.trim().to_owned()
        };
        ...
        let json = match serde_json::to_string_pretty(&snapshot) {
            Ok(json) => json,
            Err(e) => {
                self.set_error(format!("Could not serialize config: {e}"));
                return;
            }
        };
        match std::fs::write(&path, json) {
            Ok(()) => {
                if let Some(device) = self.state.device_mut_by_id(id) {
                    device.config_path = path.clone();
                }
                self.state.transient.error_message.clear();
                self.state.transient.status_message = format!("Config saved → {path}");
                self.state.push_event(format!("[{id}] config saved: {path}"));
            }
            Err(e) => self.set_error(format!("Could not write {path}: {e}")),
        }
    }

    /// Load a JSON snapshot file into `id`'s form values and push every
    /// section to the device (same step sequence as [`Self::apply_full_config`]).
    fn load_config_file(&mut self, id: &str, path: &str) {
        let path = path.trim();
        if path.is_empty() {
            self.set_error("Enter a config file path to load");
            return;
        }
        ...
        match serde_json::from_str::<ConfigSnapshotFile>(&text) {
            Ok(snapshot) => {
                if let Some(device) = self.state.device_mut_by_id(id) {
                    device.forms = snapshot.forms;
                }
                self.state.transient.error_message.clear();
                self.state.push_event(format!("[{id}] config loaded from {path}"));
                // Push the loaded config to the device right away — a loaded
                // snapshot that stays form-only silently leaves the device on
                // its previous configuration. On validation failure the form
                // keeps the loaded values so the user can correct and re-apply.
                self.apply_full_config(id);
            }
            Err(e) => self.set_error(format!("Invalid config file {path}: {e}")),
        }
    }

    /// Push every config section from `id`'s current forms to the device as
    /// one sequential step run (each step submits after the previous 2xx).
    fn apply_full_config(&mut self, id: &str) {
        let Some(device) = self.state.devices.iter().find(|d| d.id == id) else {
            return;
        };
        if let Err(message) = validate_full_config(device) {
            self.set_error(format!("Cannot apply config to {id}: {message}"));
            return;
        }
        let forms = device.forms.clone();
        let queue = full_config_steps(id, &forms);
        self.state
            .push_event(format!("[{id}] applying full configuration ({} steps)", queue.len()));
        self.start_step_queue(queue, format!("Full configuration applied to {id}"));
    }

    /// Copy `source_id`'s form values onto `id` and apply them to the device.
    fn copy_config_from(&mut self, id: &str, source_id: &str) {
        if id == source_id {
            self.set_error("Source and target device are the same");
            return;
        }
        let Some(source) = self.state.devices.iter().find(|d| d.id == source_id) else {
            self.set_error(format!("Source device not found: {source_id}"));
            return;
        };
        let forms = source.forms.clone();
        let Some(target) = self.state.device_mut_by_id(id) else {
            return;
        };
        target.forms = forms;
        self.state
            .push_event(format!("[{id}] configuration copied from {source_id}"));
        self.apply_full_config(id);
    }
    ...
    fn start_recording(&mut self, id: &str) {
        ...
        match Recorder::start(&path, &chip, Arc::clone(&self.profile)) {
            Ok(recorder) => {
    ...
    fn submit_set_wifi(&mut self, id: &str, wifi: WiFiForm) {
        ...
        // Every mode accepts a channel. In station mode it is an optional
        // pre-association hint (blank = inherit from the AP); other modes treat
        // it as the operating channel.
        let channel = {
        ...
        let body = match wifi_request_body(&wifi, channel) {
            Ok(body) => body,
            Err(message) => {
                self.set_error(message);
                return;
    ...
    fn apply_pairing_preset(
        ...
        self.start_step_queue(queue, "Pairing preset applied to both devices".to_owned());
    }

    /// Begin a sequential config-step run; each step is submitted only after
    /// the previous one succeeded (2xx).
    fn start_step_queue(&mut self, queue: Vec<(String, DeviceAction)>, done_message: String) {
        self.step_queue = queue;
        self.step_queue_done_message = done_message;
        self.run_next_step();
    }

    fn run_next_step(&mut self) {
        if let Some((id, action)) = self.step_queue.first().cloned() {
            self.process_device_action(id, action);
        }
    }

    fn advance_step_queue(&mut self, success: bool) {
        if self.step_queue.is_empty() {
            return;
        }
        if success {
            self.step_queue.remove(0);
            if self.step_queue.is_empty() {
                let message = std::mem::take(&mut self.step_queue_done_message);
                self.state.push_event(message.clone());
                self.state.transient.status_message = message;
            } else {
                self.run_next_step();
            }
        } else {
            self.step_queue.clear();
            self.step_queue_done_message.clear();
            self.state
                .push_event("Config step sequence aborted due to error".to_owned());
        }
    }
    ...
    fn handle_api_response(&mut self, response: ApiResponseEvent, followups: &mut Vec<UserIntent>) {
        ...
        self.maybe_advance_step_queue(&response);
    }

    fn maybe_advance_step_queue(&mut self, response: &ApiResponseEvent) {
        if self.step_queue.is_empty() {
            return;
        }
        ...
        if !self.step_queue.iter().any(|(id, _)| id == resp_id) {
            return;
        }
        if !response.success {
            self.advance_step_queue(false);
            return;
        }
        let Some((head_id, head_action)) = self.step_queue.first() else {
            return;
        };
        if head_id != resp_id {
            return;
        }
        if response.label == step_action_label(head_action) {
            self.advance_step_queue(true);
        }
    }
...
/// Ordered per-device steps that push every config section to the device.
///
/// Wi-Fi goes first so the mode is settled before the traffic step consults
/// it for the `unsolicited` gating (same ordering as the pairing presets).
fn full_config_steps(id: &str, forms: &DeviceForms) -> Vec<(String, DeviceAction)> {
    [
        DeviceAction::SetWifi(forms.wifi.clone()),
        DeviceAction::SetProtocol(forms.protocol),
        DeviceAction::SetTraffic(forms.traffic.clone()),
        DeviceAction::SetCsi(forms.csi.clone()),
    ...
    .into_iter()
    .map(|action| (id.to_owned(), action))
    .collect()
}

/// Validate every form field a full-config apply will submit.
///
/// Mirrors the per-action validation in [`CsiClientApp::process_device_action`]
/// / [`CsiClientApp::submit_set_wifi`]: a step that fails validation there is
/// never submitted, which would stall the sequential queue — so a loaded or
/// copied config is checked up front instead.
fn validate_full_config(device: &DeviceState) -> Result<(), String> {
    let forms = &device.forms;
    let wifi = &forms.wifi;

    if wifi.mode.requires_v07() && !device.supports_v07_modes() {
        return Err(format!(
            "mode '{}' requires esp-csi-cli-rs ≥ 0.7.0 on this device",
            wifi.mode.as_api_value()
        ));
    }
    if matches!(wifi.mode, WiFiMode::Station) {
        validate_sta_field("STA SSID", &wifi.sta_ssid)?;
        validate_sta_field("STA password", &wifi.sta_password)?;
    }
    if matches!(wifi.mode, WiFiMode::WifiAp) {
        validate_sta_field("AP SSID", &wifi.ap_ssid)?;
        validate_sta_field("AP password", &wifi.ap_password)?;
    }
    ...
    if wifi.mode.uses_peer_mac() && !wifi.peer_mac.trim().is_empty() {
        validate_peer_mac(wifi.peer_mac.trim())?;
    }
    if parse_required_u64(&forms.traffic.frequency_hz).is_none() {
        return Err("Traffic frequency must be a non-negative integer".to_owned());
    }
    if parse_required_u32(&forms.csi.val_scale_cfg).is_none() {
        return Err("val_scale_cfg must be a valid u32 number".to_owned());
    }
    if forms.phy_rate.rate.trim().is_empty() {
        return Err("PHY rate must not be empty".to_owned());
    }
    Ok(())
}

/// HTTP label emitted by [`process_device_action`] for sequential queue steps.
fn step_action_label(action: &DeviceAction) -> &str {
    match action {
        ...
        DeviceAction::SetCsi(_) => "set_csi",
        DeviceAction::SetCsiPreset(_) => "set_csi_preset",
...
/// Build the `POST …/config/wifi` body for `wifi`, sending each field only for
/// the modes whose firmware reads it. `channel` is the already-parsed channel.
fn wifi_request_body(wifi: &WiFiForm, channel: Option<u16>) -> Result<serde_json::Value, String> {
    ...
    if matches!(wifi.mode, WiFiMode::WifiAp) {
        ...
        body["ap_leases"] = json!(wifi.ap_leases);
        body["ap_burst"] = json!(wifi.ap_burst);
    }
    ...
    // `peer_mac` is the injection destination (emitter modes) or the
    // explicit peer (ESP-NOW modes); `ht40` runs the softAP as HT40
    // (`wifi-ap`) or forces the per-peer TX PHY (ESP-NOW modes). Send each
    // only where the firmware reads it.
    if wifi.mode.uses_peer_mac() {
        let peer_mac = wifi.peer_mac.trim();
        if !peer_mac.is_empty() {
            validate_peer_mac(peer_mac)?;
        }
        body["peer_mac"] = json!(peer_mac);
    }
    if wifi.mode.uses_ht40() {
        body["ht40"] = json!(wifi.ht40.as_api_value());
    }

    // The collection mode, only where the mode admits a choice (the server
    // rejects it for the modes that fix it). Unset = firmware default.
    if wifi.mode.admits_collection_choice() {
        if let Some(collection) = wifi.collection {
            body["collection"] = json!(collection.as_api_value());
        }
    }
...
mod tests {
    use super::*;
    use crate::state::{CollectionMode, Ht40Mode};

    #[test]
    fn full_config_steps_cover_every_section_in_order() {
        let forms = DeviceForms::default();
        let steps = full_config_steps("dev-a", &forms);
        let labels: Vec<&str> = steps
            .iter()
            .map(|(id, action)| {
                assert_eq!(id, "dev-a");
                step_action_label(action)
            })
            .collect();
        assert_eq!(
            labels,
    ...
    fn test_app() -> CsiClientApp {
        CsiClientApp {
            ...
            step_queue: Vec::new(),
            step_queue_done_message: String::new(),
            profile: Arc::new(StandardClientProfile),
        }
    }

    #[test]
    fn load_config_file_applies_to_device() {
        let mut app = test_app();
        app.state.devices.push(DeviceState::new("dev-a"));

        let path = std::env::temp_dir().join("csi_webclient_load_apply_test.json");
        let snapshot = ConfigSnapshotFile {
            format: 1,
        ...
        app.load_config_file("dev-a", path.to_str().unwrap());
        let _ = std::fs::remove_file(&path);

        // Loading must push the config to the device, not just fill the form:
        // the sequential queue holds one step per config section.
        assert_eq!(app.step_queue.len(), 9);
        assert!(app.state.transient.error_message.is_empty());
    }

    #[test]
    fn load_config_file_with_invalid_values_fills_form_but_reports_error() {
        let mut app = test_app();
        app.state.devices.push(DeviceState::new("dev-a"));

        let mut forms = DeviceForms::default();
        forms.traffic.frequency_hz = "fast".to_owned();
        let path = std::env::temp_dir().join("csi_webclient_load_invalid_test.json");
        let snapshot = ConfigSnapshotFile {
            format: 1,
            device_id: None,
            saved_at: None,
        ...
        // Nothing is submitted, the error is surfaced, and the loaded values
        // stay in the form so the user can correct and re-apply.
        assert!(app.step_queue.is_empty());
        assert!(!app.state.transient.error_message.is_empty());
        let device = app.state.devices.iter().find(|d| d.id == "dev-a").unwrap();
        assert_eq!(device.forms.traffic.frequency_hz, "fast");
    }

    #[test]
    fn wifi_body_sends_collection_only_where_the_mode_admits_it() {
        for mode in [
            WiFiMode::Station,
            WiFiMode::WifiAp,
            WiFiMode::EspNowCentral,
            WiFiMode::EspNowPeripheral,
        ] {
            let wifi = WiFiForm {
                mode,
                collection: Some(CollectionMode::Listener),
                ..WiFiForm::default()
            };
            let body = wifi_request_body(&wifi, None).unwrap();
            assert_eq!(body["collection"], "listener", "{mode:?}");
        }
        for mode in [
            WiFiMode::Sniffer,
            WiFiMode::Ht20Emitter,
            WiFiMode::Ht40Emitter,
            WiFiMode::EspNowSimplexSource,
            WiFiMode::EspNowSimplexPeer,
        ] {
            let wifi = WiFiForm {
                mode,
                collection: Some(CollectionMode::Listener),
            ...
            let body = wifi_request_body(&wifi, None).unwrap();
            assert!(body.get("collection").is_none(), "{mode:?}");
        }
        // Unset is omitted, leaving the firmware default.
        let body = wifi_request_body(&WiFiForm::default(), None).unwrap();
        assert!(body.get("collection").is_none());
    }

    #[test]
    fn wifi_body_sends_peer_and_ht40_in_the_esp_now_modes() {
        let wifi = WiFiForm {
            mode: WiFiMode::EspNowSimplexPeer,
            peer_mac: "aa:bb:cc:dd:ee:ff".to_owned(),
            ht40: Ht40Mode::Above,
            ..WiFiForm::default()
        };
        let body = wifi_request_body(&wifi, Some(6)).unwrap();
        assert_eq!(body["peer_mac"], "aa:bb:cc:dd:ee:ff");
        assert_eq!(body["ht40"], "above");

        let wifi = WiFiForm {
            mode: WiFiMode::EspNowCentral,
            peer_mac: "zz:zz".to_owned(),
            ..WiFiForm::default()
        };
        assert!(wifi_request_body(&wifi, None).is_err());

        // Station reads neither.
        let body = wifi_request_body(&WiFiForm::default(), None).unwrap();
        assert!(body.get("peer_mac").is_none());
        assert!(body.get("ht40").is_none());
    }

    #[test]
    fn validate_full_config_accepts_defaults() {
        let device = DeviceState::new("dev-a");
        assert!(validate_full_config(&device).is_ok());
    }

    #[test]
    fn validate_full_config_rejects_bad_fields() {
        let mut device = DeviceState::new("dev-a");
        device.forms.traffic.frequency_hz = "fast".to_owned();
        ...
        let mut device = DeviceState::new("dev-a");
        device.forms.wifi.mode = WiFiMode::WifiAp;
        assert!(validate_full_config(&device).is_err());
    }
}
